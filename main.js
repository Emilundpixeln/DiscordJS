...
class Discord {
    constructor() {
        ...
        ws.on("message", (n) => {
            var r = new DataView(n), i = r.byteLength >= 4 &&
        ...
        inflater.onEnd = (res) => {
            if (res == 0) {
                ...
                if (json.op == 10) {
                    ...
                    if (json.t == 'SESSIONS_REPLACE')
                        console.log('SESSIONS_REPLACE');
                    else if (json.t == 'PRESENCE_UPDATE')
                        console.log('PRESENCE_UPDATE');
                    else if (json.t == 'MESSAGE_CREATE')
                        console.log('MESSAGE_CREATE');
                    else if (json.t == 'MESSAGE_UPDATE')
                        console.log('MESSAGE_UPDATE');
                    else if (json.t == 'READY_SUPPLEMENTAL')
                    ...
                    else if (json.t == 'GUILD_INTEGRATIONS_UPDATE')
                        console.log('GUILD_INTEGRATIONS_UPDATE');
                    else if (json.t == 'INTEGRATION_UPDATE')
                        console.log('INTEGRATION_UPDATE');
                    else if (json.t == 'GUILD_EMOJIS_UPDATE')
    ...
    on_MESSAGE_REACTION_ADD(callback) {
        this.handler["MESSAGE_REACTION_ADD"] = callback;
    }
    edit_message(channel_id, message_id, message) {
        return __awaiter(this, void 0, void 0, function* () {
            let res = yield fetch("https://discord.com/api/v9/channels/830260317040541697/messages/899754609961230346", {
                "headers": {
                    "accept": "*/*",
                    "accept-language": "de",
...
disc.on_MESSAGE_REACTION_ADD((d) => {
    if (d.message_id != '899754609961230346')
        return;
    console.log(d);
